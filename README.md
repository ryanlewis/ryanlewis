## Hi, I'm Ryan 👋

Software Engineer in Leeds, UK. Currently at [@LHV-UK](https://github.com/LHV-UK).

Polyglot mostly writing Go, Kotlin, and TypeScript.

### Some things I've made

#### Command line

- 📢 [shout.sh](https://shout.sh) — stylised ASCII banners over curl. Try `curl shout.sh/hi`
- ✅ [things-cli](https://github.com/ryanlewis/things-cli) — Things3 on the command line
- 📋 [yat](https://github.com/ryanlewis/yat) — lightweight CLI issue tracker backed by YAML-frontmattered markdown files
- 🌈 [tint](https://github.com/ryanlewis/tint) — visual shaders for text in your terminal

#### macOS

- 🖱️ [MouseBinder](https://github.com/ryanlewis/mousebinder) — menu-bar app binding spare mouse buttons to Mission Control and friends

#### Self-hosted

- 🔔 [hubbub](https://github.com/ryanlewis/hubbub) — notification fan-out hub: one authenticated JSON API in, your channels out. Durable outbox, per-key permissions, single Go binary

#### On the web

- 🎯 [edpi.app](https://edpi.app) — Valorant pro sensitivity, DPI & gear loadouts
- 📰 [hn.rlew.io](https://hn.rlew.io) — AI-summarised RSS feed of Hacker News's best stories, article + discussion

#### Toys

- 🚽 [iss.rlew.io](https://iss.rlew.io) — the ISS Piss-o-meter: live urine-tank telemetry from the International Space Station
- 🛰️ [ioncannon.rlew.io](https://ioncannon.rlew.io) — the ISS reimagined as the GDI ion cannon from Command & Conquer
- 🫖 [Yorkshire Tea, by the numbers](https://share.rlew.io/yorkshire-tea/) — a sourced statistical case for Britain's biggest tea brand, including the findings that go the other way

### Daily drivers

[Ghostty](https://ghostty.org) · [Herdr](https://herdr.dev) · [Neovim](https://neovim.io) · [lazygit](https://github.com/jesseduffield/lazygit) · [mise](https://mise.jdx.dev) · [chezmoi](https://www.chezmoi.io) (see [dotfiles](https://github.com/ryanlewis/dotfiles)) · [Claude Code](https://claude.com/claude-code) · [Things](https://culturedcode.com/things/) · [Obsidian](https://obsidian.md)

### Elsewhere

- 🌐 [rlew.io](https://rlew.io)
- 💼 [LinkedIn](https://www.linkedin.com/in/ryan-lewis/)
